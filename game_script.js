...
function recordGuess(guess) {
    ...
    if (guess === 'H') {
        choiceMessage.textContent = "BLUE";
        choiceMessage.style.color = "#008CBA";
        document.querySelector('.guessButton.T').classList.remove('T');
    } else {
        choiceMessage.textContent = "RED";
        choiceMessage.style.color = "#f44336";
...
function revealResult(action, a) {
    ...
    if (guess === P2guess) {
        if (guess === flipResult) {
            ...
            gameMessage.textContent = "Oh no! YOU LOST. Better luck next time!";
            };
        } else {
            gameMessage.textContent = "Your guesses mismatched. IT'S A DRAW.";        
    };
    setTimeout(() => {
